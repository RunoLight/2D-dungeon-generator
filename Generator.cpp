...
{
    for (int i = newRoom.x1; i < newRoom.x1 + newRoom.height - 1; i++)
        for (int j = newRoom.y1; j < newRoom.y1 + newRoom.width - 1; j++)
...
void Generator::Print()
{
    ...
    {
        for (int j = 0; j < Generator::map.size(); j++)
        {
...
void Generator::placeRooms()
{
    ...
    for (int i = 0; i < RoomPlaceTries; i++)
    {
        ...
        int x = rand() % (map.size() - w - 1) + 1;
        int y = rand() % (map.size() - h - 1) + 1;
